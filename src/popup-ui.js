  ...
  try {
    const initUi = () => {
      ...
      if (openOptionsBtn) {
        ...
        if (!openOptionsBtn.textContent || openOptionsBtn.textContent.trim() === '') {
          openOptionsBtn.textContent = 'Options';
        }
      }
      ...
      try {
        storage.get({ pinnedWindowId: null }, (s) => { updatePinUi(!!(s && s.pinnedWindowId)); });
      } catch (e) {}
        ...
        pinBtn.addEventListener('click', async () => {
          try {
            storage.get({ pinnedWindowId: null }, async (s) => {
              ...
              if (winId) {
                try {
                  await new Promise((res) => window.$host.windows.remove(winId, () => res()));
                } catch (e) {}
                ...
                try {
                  window.$host.windows.create({ url, type: 'popup', width: 420, height: 640 }, (created) => {
                  ...
                  try { window.$host.tabs.create({ url }); } catch (err) {}
                }
              }
            });
          } catch (e) {
            console.warn('pin handler error', e);
          }
        });
      }
      ...
      try {
        storage.get({ mqtt_ctrl_enabled: false, mqtt_ctrl_broker: {}, mqtt_broker: {} }, (s) => {
          ...
          const st = (typeof s.mqtt_ctrl_enabled !== 'undefined')
            ? (s.mqtt_ctrl_enabled ? 'enabled' : 'disabled')
            : (s.mqtt_enabled ? 'enabled' : 'disabled');
          const url = (broker && broker.brokerUrl) ? broker.brokerUrl : 'ws://localhost:9001';
          const el = document.getElementById('mqtt-status');
          if (el) el.textContent = `MQTT: ${st} — ${url}`;
        });
      } catch (e) {
        if (typeof rcLog !== 'undefined') rcLog('error', 'failed to read mqtt status', e && e.message ? e.message : e);
      }
    };
